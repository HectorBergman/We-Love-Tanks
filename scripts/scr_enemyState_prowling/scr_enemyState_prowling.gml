function enemyState_prowling(){
	...
	for (var i = 0; i < instance_number(obj_breadCrumbs); ++i;){
		...
		if reachable{
			...
			if (crumbDistance) < nearestCrumbDistance && !collision_line(breadCrumb.x,breadCrumb.y,playerTank.x,playerTank.y,obj_wall,0,1){ //!collision_cone(breadCrumb.x,breadCrumb.y,playerTank.x,playerTank.y,45, 5, obj_wall){
